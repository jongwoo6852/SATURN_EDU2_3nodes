...
subroutine Main()
    ...
    double precision, allocatable :: U(:)       ! Displacement vector
    ...
    write(0, "(a)"), "   Ref. energy =  1.0760861791E-04"
    write(0, "(a)")
...
subroutine Displacement_Stress(node, elem, prop, R, U)
    ...
    do i = 1, size(elem)
        ...
        write(4, "(1x,28(1P,E13.5))") eNode(1,:), displace(1), displace(4), Stress(1,:),&
                                      eNode(2,:), displace(2), displace(5), Stress(2,:),&
                                      eNode(3,:), displace(3), displace(6), Stress(3,:)
    end do
...
subroutine Prob_Cook_Skew_Beam(node, elem, prop)
    ...
    do j = 1, n_j_elem
        do i = 1, n_i_elem
            ...
            elem(index)%cn(2) = n_i_node * j + i + 1
            elem(index)%cn(3) = n_i_node * j + i
            
            index = n_domain * n_i_elem + n_domain * (j - 1) + i
            elem(index)%cn(1) = n_i_node * (j - 1) + i
