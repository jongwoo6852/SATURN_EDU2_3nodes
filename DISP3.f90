...
! by Jongwoo Han (jongwoo6852@jbnu.ac.kr)
!
!               s
!               l
!               3
...
! This is program is free software: you can redistribute it and/or modify it under
...
! Stiffness matrix (u1, u2, u3, v1, v2, v3)
function Plane_Stiffness(Young, Poisson, thick, node) result(Ke)
    ...
    do i = 1, nGP
        ...
        call Gauss3_Point(i, r, s, weight)
...
function Plane_Stress(Young, Poisson, node, disp) result(stress)
    ...
    double precision :: stress(nNPE, 3)     ! Stress (Sxx, Syy, Sxy)
    ...
    do i = 1, nGP
        ...
        ! Stresses
        stress(i, :) = matmul(matmul(C, B), disp)
    end do
...
subroutine Material_Law(Young, Poisson, C)
    ...
    C(2,2) = Young / (1.0d0-Poisson**2.0d0)
    C(1,2) = Poisson * C(1,1)
    C(2,1) = C(1,2)
...
subroutine Gauss3_Point(i, r, s, weight)
    integer, intent(in) :: i
    double precision, intent(out) :: r, s, weight

    double precision :: GaussPoint(3), w(3)
    data GaussPoint / 0.1666666667d0, 0.666666667d0, 0.166666667d0 /
    data w         / 0.333333333d0, 0.333333333d0, 0.333333333d0 /
...
! dHxy matrix. dHxy(1,:) = dH/dx, dH(2,:) = dHxy/dy
subroutine dHxy_Matrix(r, s, node, det_j, dHxy)
    ...
    ! Derivate of shape function, dH/dr, dH/ds
